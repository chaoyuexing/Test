...
package com.linkage.lib.database;
...
import com.linkage.lib.database.sqlite.*;
import com.linkage.lib.database.table.*;
import com.linkage.lib.exception.DbException;
import com.linkage.lib.util.IOUtils;
...
public class DbUtils {
    ...
    /**
     * 该方法需要在所有数据库操作的finally里调用 否则writeLock无法释放 将会导致异常
     */
    ...
    public void insertOrUpdateInstance(String tableName, ContentValues values, String uniqueColumn, String[] uniqueArgs) 
    {
    	try {
            beginTransaction();
            Cursor cursor = null;
            boolean exist = false;
            try {
            	cursor = database.query(tableName, null, uniqueColumn + "=?", uniqueArgs, null, null, null);
            	if (cursor != null && cursor.moveToFirst()) exist = true;
			} catch (Exception e) {
				LogUtils.e("DbUtils", e);
			}
            	finally {
            ...
            try {
            	if (exist) {
                	database.update(tableName, values, uniqueColumn + "=?", uniqueArgs);
    			} else {
