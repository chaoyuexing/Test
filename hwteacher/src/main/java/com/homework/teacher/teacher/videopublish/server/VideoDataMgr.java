...
//    private ArrayList<SuperPlayerModel> mDefaultList;
    ...
    public void getPublishSig() {
        String url = Consts.SERVER_TENCENT_SIGN;
        String relative_url = url.replace(Consts.SERVER_IP, "");
        String sign_body = "";
        WDStringRequest mRequest = new WDStringRequest(com.android.volley.Request.Method.GET, url, relative_url, sign_body, false, response -> {
                    Simple simple = new Simple().getFromGson(response);
                    if (simple != null && simple.getCode() == Consts.REQUEST_SUCCEED_CODE) {
                        notifyGetPublishSigSuccess(simple.data);
                    } else {
                        notifyGetPublishSigFail(Const.RetCode.CODE_PARSE_ERR);
                    }
                }, arg0 -> StatusUtils.handleError(arg0, BaseApplication.mContext));
        BaseApplication.getInstance().addToRequestQueue(mRequest, TAG);
//        String sigParams = getSigParams();
//        Request request = new Request.Builder()
//                .url(Consts.SERVER_IP + Consts.SERVER_TENCENT_SIGN)
//                .build();
